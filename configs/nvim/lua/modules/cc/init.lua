-- https://github.com/skanehira/dotfiles/blob/master/vim/lua/modules/ai/init.lua
-- AIツール (Claude) のtmux統合モジュール
-- 公開APIとコマンド登録を提供

local tmux = require("modules.cc.tmux")
local buffer = require("modules.cc.buffer")
...
local function open_input_buffer(tool_name, args)
  ...
  if not tmux.is_in_tmux() then
    vim.notify(
      "エラー: このコマンドはtmuxセッション内でのみ使用できます",
